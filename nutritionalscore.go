...
func (p ProetinGram) GetPoints(st ScoreType) int {
...
func GetNutritionalScore(n NutritionalData, st ScoreType) NutritionalScore {
	...
	if st != Water {
		...
		negative = n.Energy.GetPoints(st) + n.Sugars.GetPoints(st) + n.SaturatedFattyAcids.GetPoints(st) + n.Sodium.GetPoints(st) // bad for body
		positive = fruitPoints + fibrePoints + n.Protein.GetPoints(st)                                                            // good for body
